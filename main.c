...
bool start=false,list=false,quit=false,help=false;
bool back=false;
bool retrn=false,wRestart=false;

float distPlay=0,distRestart=0,distPReturn=0;
float distStart=0,distExit=0,distList=0,distHelp=0;
float distBack=0;
...
void display()
{
	...
	if(state==START){
		...
		if(help==true)
		{
			glColor4ub(255,255,255,2-distHelp*2/(height*height/(15*15)));
...
void mouse(int btn,int st,int x,int y)
{
	...
	if(state==START)
	{
		...
		distHelp=(552-x)*(552-x)+(610-y)*(610-y);
		if(distHelp<height*height/(15*15))
		{
			if(btn==GLUT_LEFT_BUTTON && st==GLUT_DOWN)
				state=HELP;
		}		
...
void mouseMove(int x,int y)
{
	if(state==START)
	{
		...
		////////for Help Icon/////
		distHelp=(550-x)*(550-x)+(610-y)*(610-y);
		if(distHelp<height*height/(15*15))
		{
			help=true;
		}else
